...
struct Settings {
   ...
   [[nodiscard]] std::set<std::string> getDefines() const {
      ...
      if (enableRaymarching) defines.insert("ENABLE_RAYMARCHING");
      if (enableSoftMarch) defines.insert("ENABLE_SOFT_MARCH");
      if (enableTriplanar) defines.insert("ENABLE_TRIPLANAR");
      if (enableBlending) defines.insert("ENABLE_BLENDING");
      if (debugView == 1) defines.insert("DEBUG_NORMAL");
      else if (debugView == 2) defines.insert("DEBUG_HEIGHT");
      return defines;
   }
};
