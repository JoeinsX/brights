...
public:
    explicit World(TileRegistry& registry, std::mt19937 rng, WorldGenerator& worldGenerator, uint32_t loadingRadius, uint32_t unloadingThreshold)
        ...
        , unloadingThreshold(unloadingThreshold)
        , registry(registry)
...
private:
    struct TaskResult {
        enum class Type { Generated, Meshed } type;
    ...
    void tryQueueMeshing(glm::ivec2 pos) {
        if(pendingMeshing.contains(pos)) return;
...
public:
    WorldRenderAdapter(wgpu::Queue queue, wgpu::Buffer chunkOffsetBuffer, wgpu::Buffer chunkDataBuffer, wgpu::Buffer tilemapBuffer, uint32_t renderRadius, World& world,
                       Camera& camera)
        : queue(queue)
        , chunkOffsetBuffer(chunkOffsetBuffer)
        ...
        for(int y = 0; y < renderRadius * 2; ++y) {
            for(int x = 0; x < renderRadius * 2; ++x) {
                ...
                if(chunk->needsReloading) {
                    queue.writeBuffer(chunkDataBuffer, chunk->getPackedData().size() * sizeof(uint16_t) * idx1, chunk->getPackedData().data(),
                                      chunk->getPackedData().size() * sizeof(uint16_t));

                    queue.writeBuffer(tilemapBuffer, chunk->getDisplayData().size() * sizeof(uint8_t) * idx1, chunk->getDisplayData().data(),
